...
function maxResult(maxIndex){
	if(maxIndex == 1){
		return 'a news source using neutral language'
	}
	if(maxIndex == 2){
		return 'an AI source using emotional tone'
	}
	if(maxIndex == 3){
		return 'a news source using emotional tone'
	}
	if(maxIndex == 4){
...
credibilityRatings[0] = -1;
    ...
    console.log("Rated post index:", maxIndex);
	  ...
	  //override to include summary details 
	    ...
	    textStyle(BOLD);
	  fill(188, 71, 73);
text( `You found ${maxResult(maxIndex)} most credible.`,
  650, 330, 500, 400);
	  textStyle(NORMAL);
	  fill(0);
	  text( 'What led to that decision?',
  650, 400, 505, 400);
